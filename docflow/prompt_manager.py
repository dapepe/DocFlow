    ...
    def _load_prompt_template(self) -> str:
        ...
        try:
            ...
            if prompt_path.exists():
                ...
                logger.warning(f"Prompt file {self.prompt_file} not found, using default template")
...
3. Provide ONLY the JSON response, no additional text"""
    ...
    def _generate_context_hints(self, document_type: Optional[str] = None, 
        ...
        if file_extension:
            ext_hints = {
                '.pdf': ["May contain multiple pages, check for continuation across pages"],
                '.jpg': ["Image quality may affect OCR accuracy, focus on clear text regions"],
        ...
        return "- " + "\n- ".join(hints) if hints else "Use general document analysis best practices"
    ...
    def generate_prompt(self, 
                       ...
                       model_specific_instructions: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            if model_specific_instructions:
                placeholders['model_instructions'] = model_specific_instructions
                # Add model instructions placeholder to template if not present
                if '{model_instructions}' not in self.template:
                    self.template += "\n\n**Model-Specific Instructions:**\n{model_instructions}"
            
            # Substitute placeholders
            formatted_prompt = self.template.format(**placeholders)
