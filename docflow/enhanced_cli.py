...
class EnhancedCLI:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'preferred_models': os.getenv('DOCFLOW_PREFERRED_MODELS', 'openrouter-gemini-flash,qwen-vision,fallback').split(',')
        }

    def smart_model_selection(self, file_path: str, user_model: Optional[str] = None) -> str:
        ...
        if file_ext in ['.jpg', '.jpeg', '.png'] or file_size < 100000:  # Images or small files
        ...
        elif any(lang in file_path.lower() for lang in ['chinese', 'japanese', 'korean', 'zh', 'ja', 'ko']):
    ...
    def auto_detect_ocr_need(self, file_path: str) -> bool:
        ...
        if self.config['auto_ocr'] == 'false':
            ...
            file_ext = Path(file_path).suffix.lower()
            return file_ext in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.tiff', '.bmp']
...
@click.option('--show-performance', '-p', is_flag=True, help="Show model performance stats")
...
def enhanced_models(output_format='detailed', show_performance=False, test_connection=False):
    ...
    for model_id, description in models_dict.items():
        ...
        elif model_id in ['qwen-vision', 'granite-vision', 'gemma', 'llava', 'llama-vision']:
            ...
            for model_id, desc in model_list:
                ...
                if test_connection:
                    ...
                    try:
                        ...
                        is_available = model_class.is_available() if model_class else False
...
@click.option('--formats', multiple=True, help="File formats to process", 
...
def batch(path, model, output_dir, parallel, formats, recursive, resume):
    ...
    enhanced_batch_processing(path, model, output_dir, parallel, formats, recursive, resume)

def enhanced_batch_processing(path, model=None, output_dir=None, parallel=4, formats=None, recursive=False, resume=False):
    ...
    if not files:
        console.print(f"[yellow]⚠️  No files found matching formats: {', '.join(formats)}[/yellow]")
        return
    ...
    if not output_dir:
        output_dir = path / 'docflow_output' if path.is_dir() else path.parent / 'docflow_output'
    else:
    ...
    if resume:
        ...
        if progress_file.exists():
            ...
            console.print(f"[blue]Resuming: {len(processed_files)} files already processed[/blue]")
    ...
    with Progress(
        ...
        def process_single_file(file_path):
            try:
                ...
                selected_model = model or cli_helper.smart_model_selection(str(file_path))
...
def generate_batch_summary(results: List[Dict], failed_files: List[Dict], output_dir: Path):
    ...
    summary_table.add_row("Successful", f"{summary['successful']} ({summary['success_rate']:.1f}%)")
...
def display_enhanced_results(result: Dict[str, Any], filename: str, include_text: bool = False):
    """Display processing results in an enhanced format"""
    # Main results panel
    console.print(Panel(f"[bold cyan]📄 Analysis Results: {filename}[/bold cyan]", expand=False))
    ...
    if ai_analysis:
        ...
        info_table.add_row("Processing Status", "✅ Success" if ai_analysis.get('success') else "❌ Failed")
        
        if 'validation_passed' in ai_analysis:
            validation_status = "✅ Passed" if ai_analysis['validation_passed'] else "⚠️  Issues Found"
        ...
        if meta:
            ...
            if 'amounts' in meta and meta['amounts']:
                ...
                for key, value in meta['amounts'].items():
                    if value is not None:
                        console.print(f"    • {key}: ${value:,.2f}" if isinstance(value, (int, float)) else f"    • {key}: {value}")
            ...
            if 'entities' in meta and meta['entities']:
                ...
                for entity_type, entities in meta['entities'].items():
                    if entities:
                        console.print(f"    • {entity_type}: {', '.join(entities) if isinstance(entities, list) else entities}")
...
def show_performance_stats():
    ...
    if cache_stats:
        ...
        cache_table.add_row("Current Size", f"{cache_stats.get('size', 0)} / {cache_stats.get('max_size', 0)}")
    ...
    system_table.add_row("Prompt Template", "✅ Valid" if prompt_valid else "❌ Invalid")
...
def interactive_config():
    ...
    if choice == "1":
        ...
        new_model = Prompt.ask("Choose new default model", choices=available_models, default=current_model)
        
        # Here you would update the .env file
        console.print(f"[green]✅ Default model set to: {new_model}[/green]")
        console.print("[yellow]💡 Update your .env file: PRIMARY_MODEL={new_model}[/yellow]")
...
def show_help_menu():
    ...
    console.print(Panel("[bold cyan]❓ DocFlow Help & Examples[/bold cyan]", expand=False))
    ...
    examples = [
        ...
        ("Custom Output", "docflow process receipt.jpg --output analysis.json --include-text"),
