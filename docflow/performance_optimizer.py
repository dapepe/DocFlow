...
class PerformanceCache:
    ...
    def set(
        self, text: str, model_name: str, schema: Dict[str, Any], result: Dict[str, Any]
    ):
...
class PerformanceMonitor:
    ...
    def record_request(
        self, model_name: str, duration: float, success: bool, text_length: int
    ):
        ...
        with self.lock:
            ...
            for model_name, stats in self.metrics.items():
                ...
                results[model_name] = {
                    ...
                    "average_duration": stats["total_duration"] / successful_requests
                    ...
                    "average_text_length": stats["total_text_length"] / total_requests
                    if total_requests > 0
                    else 0,
...
class BatchProcessor:
    ...
    def process_batch(
        self, documents: List[Dict[str, Any]], processor_func: Callable, **kwargs
    ) -> List[Dict[str, Any]]:
        """Process multiple documents concurrently"""
        ...
        for doc, future in futures:
            try:
                ...
                logger.error(
                    f"Batch processing failed for document {doc.get('id', 'unknown')}: {e}"
                )
                results.append(
                    {
                        "success": False,
...
def cached_model_response(func):
    ...
    def wrapper(self, text: str, image_path: Optional[str] = None, **kwargs):
        ...
        if image_path is None:
            cached_result = performance_cache.get(
                text, self.__class__.__name__, self.schema
            )
            if cached_result:
        ...
        try:
            ...
            if success and image_path is None:
                performance_cache.set(
                    text, self.__class__.__name__, self.schema, result
                )
...
def cached_model_response_async(func):
    """Async decorator to cache model responses"""

    @wraps(func)
    async def wrapper(self, text: str, image_path: Optional[str] = None, **kwargs):
        # Try cache first (only for text-based requests to avoid image caching complexity)
        if image_path is None:
            cached_result = performance_cache.get(
                text, self.__class__.__name__, self.schema
        ...
        start_time = time.time()
        try:
            result = await func(self, text, image_path, **kwargs)
            duration = time.time() - start_time
            success = result.get("success", False)
