...
class DocumentWorkflow:
    ...
    def _execute_workflow_steps(self, config: Dict[str, Any], input_path: str) -> Dict[str, Any]:
        """Execute workflow steps in sequence"""
        results = {'workflow_results': [], 'final_output': None}
        
        for i, step in enumerate(config.get('steps', [])):
            console.print(f"[blue]Step {i+1}: {step.get('name', 'Unnamed step')}[/blue]")
        ...
        results['final_output'] = results['workflow_results'][-1] if results['workflow_results'] else None
        return results
    
    def _execute_step(self, step: Dict[str, Any], input_path: str, context: Dict[str, Any]) -> Dict[str, Any]:
...
class SmartDocumentRouter:
    ...
    def route_document(self, file_path: str, preview_text: str = None) -> Dict[str, Any]:
        ...
        return best_route or {
            'name': 'default',
            'config': {'model': 'qwen-vision', 'ocr': 'auto', 'output_dir': './processed/default'},
            'confidence': 0.1
        }
    
    def _calculate_route_score(self, file_path: Path, preview_text: str, route_config: Dict) -> float:
        ...
        if 'keywords' in route_config and preview_text:
            ...
            matched_keywords = sum(1 for keyword in route_config['keywords'] 
...
class DocumentAnalytics:
    ...
    def _update_aggregations(self, record: Dict[str, Any]):
        ...
        self.data['document_types'][doc_type] = self.data['document_types'].get(doc_type, 0) + 1
    ...
    def generate_insights_report(self) -> Dict[str, Any]:
        ...
        recent_data = [
            record for record in self.data['processing_history']
        ...
        insights = {
            'summary': {
                ...
                'overall_success_rate': sum(1 for r in self.data['processing_history'] if r.get('success')) / len(self.data['processing_history']),
                'most_common_document_type': max(self.data['document_types'], key=self.data['document_types'].get) if self.data['document_types'] else None
            },
    ...
    def _generate_optimization_suggestions(self) -> List[str]:
        ...
        ocr_docs = [r for r in self.data['processing_history'] if 'jpg' in r.get('file_name', '').lower() or 'png' in r.get('file_name', '').lower()]
        if len(ocr_docs) > 10:
            suggestions.append("Consider using openrouter-pixtral for image documents - it's optimized for OCR")
        
        # Check for large document processing
        large_docs = [r for r in self.data['processing_history'] if r.get('text_length', 0) > 50000]
        if len(large_docs) > 5:
            suggestions.append("For large documents, consider using openrouter-gemini-pro with its large context window")
        
        # Check processing time patterns
        slow_processing = [r for r in self.data['processing_history'] if r.get('processing_time', 0) > 30]
        if len(slow_processing) > len(self.data['processing_history']) * 0.2:
            suggestions.append("Consider using faster models like openrouter-gemini-flash for routine processing")
...
@workflow.command()
@click.argument('name')
@click.option('--template', type=click.Choice(['invoice', 'contract', 'receipt', 'custom']), 
              default='custom', help="Workflow template")
...
def list():
    ...
    if not workflows:
        console.print("[yellow]No workflows found. Create one with 'docflow workflow create <name>'[/yellow]")
        return
...
@click.option('--dry-run', is_flag=True, help="Show routing decisions without processing")
def smart_route(path, dry_run):
    ...
    if path.is_file():
        ...
        files = [f for f in path.rglob("*") if f.is_file() and f.suffix.lower() in ['.pdf', '.docx', '.txt', '.jpg', '.png']]
    
    console.print(f"[blue]Analyzing {len(files)} documents for intelligent routing...[/blue]")
...
def insights():
    ...
    summary_table.add_row("Total Documents Processed", str(summary['total_documents_processed']))
    summary_table.add_row("Recent Documents (30 days)", str(summary['recent_documents']))
    summary_table.add_row("Overall Success Rate", f"{summary['overall_success_rate']:.1%}")
    summary_table.add_row("Most Common Document Type", summary['most_common_document_type'] or 'N/A')
