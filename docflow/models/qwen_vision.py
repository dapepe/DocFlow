    ...
    def extract_information(self, text: str, image_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(response, dict) and 'error' not in response:
                ...
                is_valid, errors, corrected_response = self.validator.validate_response(response)
                ...
                if not is_valid:
                    logger.warning(f"Qwen Vision response validation issues: {errors}")
                
                return result
            else:
                logger.error(f"Invalid response from Qwen Vision: {response}")
                return {
                    "success": False,
            ...
            logger.error(f"Error in Qwen Vision analysis: {e}")
            return {
                "success": False,
                "error": str(e),
                "model_name": self.model
